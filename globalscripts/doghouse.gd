...
func _physics_process(delta):
	if is_hidden and $Camera3D:
		...
		var right_dir = $Camera3D.transform.basis.x
		right_dir.y = 0
		right_dir = right_dir.normalized()

		var forward_dir = -$Camera3D.transform.basis.z
		forward_dir.y = 0
...
func hide_exit():
	...
	if exit_marker:
		...
		if player.HEAD:
			...
			var head_rot = player.HEAD.rotation
	...
	var raycast_node = player.HEAD.get_node_or_null("RayCast3D")
	if raycast_node:
